...
(function($){
	$.fn.portfolioSetter=function(options){
		...
		if (windowSize <= 479) {
            var defaults={
				...
				pageWidth: 300,  //the width of each page
				pageHeight:255,  //the height of each page
		...
		else if ((windowSize<=1000) && (windowSize>=740)) {
            var defaults={
				//default settings
				itemsPerPage: 12, //the number of items per page
				pageWidth: 760,  //the width of each page
				pageHeight:330,  //the height of each page
				itemMargin:30,  //margin of each of the portfolio items
				showCategories: true,  // if set to false, the categories will be hidden
				allText: 'ALL', //the ALL text displayed before the categories names
				easing: 'easeOutExpo', //the animation easing
            ...
            var defaults={
				//default settings
				itemsPerPage: 12, //the number of items per page
				pageWidth: 1024,  //the width of each page
				pageHeight:455,  //the height of each page
				itemMargin:30,  //margin of each of the portfolio items
				showCategories: true,  // if set to false, the categories will be hidden
				allText: 'ALL', //the ALL text displayed before the categories names
				easing: 'easeOutExpo', //the animation easing
		...
		var categories=[], items=[], pageWrappers=[], imagesLoaded=0, counter=0, ie=false, categoryHolder, ie11=false;
			...
			success:function(xml){
				...
				$(xml).find('item').each(function(i){
					...
					var item = {
						...
						category:cat,
						description:desc,
						obj:$('<div class="'+options.itemClass+'"><a rel="lightbox['+parentId+']" class="single_image" href="http://graphic-aid.com/'+prev+'" title="'+desc+'"><img src="http://graphic-aid.com/'+thum+'" /></a></div>')
					};
					items.push(item);
				});
	...
	function setSetter(){
		...
		//check for IE11 or newer
		ie11 = !!navigator.userAgent.match(new RegExp("Trident/.*rv:([0-9]{1,}[\.0-9]{0,})"));
	...
	function displayItems(){
		...
		$('a[rel="lightbox['+parentId+']"]').prettyPhoto({theme:'dark_rounded', keyboard_shortcuts: false, social_tools:'', animation_speed:'normal', slideshow:false, overlay_gallery:false});
